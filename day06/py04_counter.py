...
font = fnt.Font(family='NanumGothic', size=20)

# 라벨. 숫자카운트를 표시할 라벨
label = Label(root, text='버튼 클릭: 0', fg = 'blue', font=font)
# side = LEFT, TOP, RIGHT, BOTTOM
# padding = 안쪽 여백, padx(왼쪽, 오른쪽에 여백), pady(위, 아래 여백)
label.pack(side = TOP, pady=20)
# 버튼, command 파라미터 - 이벤트 함수를 정의
buttonUp = Button(root, text='카운트증가', 
                  font = font,
                  command=countUp) # countUp이라는 함수가 마우스 클릭때마다 실행
buttonUp.pack(side=LEFT, padx=20,pady=20)
buttonInit = Button(root, text='초기화', font = font, command=countInit) # countInit 함수가 실행
