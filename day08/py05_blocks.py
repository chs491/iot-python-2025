...
class Block:
    ...
    def move(self): # 볼 무브 
        # 볼이 움직이는 x축 값을 계속 계산하려면 x는 dir 값을 라디안으로 변환 후 코사인처리
        self.rect.centerx += math.cos(math.radians(self.dir)) * self.speed
        # 볼이 움직이는 y축 값을 계속 계산하려면 y는 dir 값을 라디안으로 변환 후 사인처리   
...
def main():
    ...
    score = 0
    BLOCK = []
    BALL = Block((200,200,0), Rect(375, 650, 20, 20), 10) # 공 생성, 공 스피드 10으로 설정
    PADDLE = Block((200,200,0), Rect(375,700,120,30)) # 공을 맞출 페달을 생성
    # 클래스 생성
    # 무지개색 정보
    ...
    while True:
        ...
        M_SPEED = smallFont.render(f'SCORE : {BALL.speed}', True, 'White')
        Surface.fill((0,0,0))
            ...
            if event.type == QUIT: 
                ...
                if event.key == K_LEFT:
                    if PADDLE.rect.centerx < 50:
                        ...
                        PADDLE.rect.centerx -= 10 # 페달은 왼쪽 오른쪽으로만 이동
                elif event.key == K_RIGHT:
                    if PADDLE.rect.centery > (SCREEN_WIDTH - 50):
        ...
        if is_game_start == False:
            ...
            if BALL.rect.centery > 800:
                Surface.blit(M_FAIL, (((SCREEN_WIDTH / 2) - (400/2),(SCREEN_HEIGHT/2) - (50/2))))
